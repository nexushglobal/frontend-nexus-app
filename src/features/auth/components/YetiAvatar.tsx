...
// Lightweight port of the example CodePen animation with GSAP 3.
// Notes:
// - We avoid MorphSVG and instead toggle visibility of mouth/body variants.
// - Eyes follow the email caret; arms cover eyes on password focus and lower on blur.
// - Toggling "show password" rotates two fingers to reveal one eye.
const YetiAvatar = forwardRef<YetiAvatarHandle, Props>(function YetiAvatar(
  { emailInputRef, passwordInputRef, className },
  ref,
) {
  const containerRef = useRef<HTMLDivElement | null>(null);

  // SVG element refs resolved at runtime via querySelector within the container
  const els = useRef({
    mySVG: null as SVGSVGElement | null,
    twoFingers: null as SVGGElement | null,
    armL: null as SVGGElement | null,
    armR: null as SVGGElement | null,
    eyeL: null as SVGGElement | null,
    eyeR: null as SVGGElement | null,
    nose: null as SVGPathElement | null,
    mouth: null as SVGGElement | null,
    mouthBG: null as SVGPathElement | null,
    mouthSmallBG: null as SVGPathElement | null,
    mouthMediumBG: null as SVGPathElement | null,
    mouthLargeBG: null as SVGPathElement | null,
    mouthMaskPath: null as SVGPathElement | null,
    tooth: null as SVGPathElement | null,
    tongue: null as SVGGElement | null,
    chin: null as SVGPathElement | null,
    face: null as SVGPathElement | null,
    eyebrow: null as SVGGElement | null,
    outerEarL: null as SVGGElement | null,
    outerEarR: null as SVGGElement | null,
    earHairL: null as SVGGElement | null,
    earHairR: null as SVGGElement | null,
    hair: null as SVGPathElement | null,
    bodyBG: null as SVGPathElement | null,
    bodyBGchanged: null as SVGPathElement | null,
  });

  const state = useRef({
    activeElement: null as 'email' | 'password' | null,
    screenCenter: 0,
    svgRect: { x: 0, y: 0, w: 0, h: 0 },
    emailRect: { x: 0, y: 0, w: 0, h: 0 },
    emailScrollMax: 0,
    eyeScale: 1,
    blinkingTween: null as gsap.core.Tween | null,
    eyesCovered: false,
  });
  ...
  const getPosition = (el: HTMLElement) => el.getBoundingClientRect();

  const recalcLayout = () => {
    const c = containerRef.current;
    const email = emailInputRef?.current;
    if (!c || !els.current.mySVG || !email) return;
    const svgR = getPosition(c);
    const emailR = getPosition(email);
    state.current.svgRect = {
      x: svgR.left,
      y: svgR.top,
      w: svgR.width,
      h: svgR.height,
    };
    state.current.emailRect = {
      x: emailR.left,
      y: emailR.top,
      w: emailR.width,
      h: emailR.height,
    };
    state.current.screenCenter = svgR.left + svgR.width / 2;
    state.current.emailScrollMax = email.scrollWidth;
  };

  const startBlinking = (maxDelay = 5) => {
    const delay = Math.floor(Math.random() * Math.max(1, maxDelay));
    const eyes = [els.current.eyeL, els.current.eyeR].filter(
      Boolean,
    ) as Element[];
    state.current.blinkingTween?.kill();
    state.current.blinkingTween = gsap.to(eyes, {
      delay,
      duration: 0.1,
      scaleY: 0,
      yoyo: true,
  ...
  const stopBlinking = () => {
    state.current.blinkingTween?.kill();
    state.current.blinkingTween = null;
    const eyes = [els.current.eyeL, els.current.eyeR].filter(
      Boolean,
    ) as Element[];
    gsap.set(eyes, { scaleY: state.current.eyeScale });
  };
  ...
  const resetFace = () => {
    ...
    gsap.to([eyeL, eyeR].filter(Boolean), {
      duration: 1,
      x: 0,
      y: 0,
      ease: 'expo.out',
    ...
    gsap.to([face, eyebrow].filter(Boolean), {
      duration: 1,
      x: 0,
      y: 0,
      skewX: 0,
    ...
    gsap.to([outerEarL, outerEarR, earHairL, earHairR, hair].filter(Boolean), {
      duration: 1,
      x: 0,
      y: 0,
      scaleY: 1,
  ...
  const calculateFaceMove = () => {
    ...
    const eyeL = els.current.eyeL!;
    const eyeR = els.current.eyeR!;
    const nose = els.current.nose!;
    const mouth = els.current.mouth!;
    const chin = els.current.chin!;
    const face = els.current.face!;
    const eyebrow = els.current.eyebrow!;
    const outerEarL = els.current.outerEarL!;
    const outerEarR = els.current.outerEarR!;
    const earHairL = els.current.earHairL!;
    const earHairR = els.current.earHairR!;
    const hair = els.current.hair!;

    // Compute a proportional caret position across the input width
    const emailR = getPosition(email);
    const style = getComputedStyle(email);
    const padL = parseFloat(style.paddingLeft || '0');
    const padR = parseFloat(style.paddingRight || '0');
    const contentW = Math.max(1, emailR.width - padL - padR);
    const caretIndex = email.selectionEnd ?? email.value.length;
    const len = Math.max(1, email.value.length);
    const ratio = Math.min(1, Math.max(0, caretIndex / len));
    const targetX = emailR.left + padL + ratio * contentW;
    const targetY = emailR.top + emailR.height * 0.38;

    const svgR = state.current.svgRect;
    const eyeLPos = { x: svgR.x + 84, y: svgR.y + 76 };
    const eyeRPos = { x: svgR.x + 113, y: svgR.y + 76 };
    const nosePos = { x: svgR.x + 97, y: svgR.y + 81 };
    const mouthPos = { x: svgR.x + 100, y: svgR.y + 100 };

    const eyeLAngle = getAngle(eyeLPos.x, eyeLPos.y, targetX, targetY);
    const eyeRAngle = getAngle(eyeRPos.x, eyeRPos.y, targetX, targetY);
    const noseAngle = getAngle(nosePos.x, nosePos.y, targetX, targetY);
    const mouthAngle = getAngle(mouthPos.x, mouthPos.y, targetX, targetY);
    ...
    gsap.to(eyeL, { duration: 0.5, x: -eyeLX, y: -eyeLY, ease: 'expo.out' });
    gsap.to(eyeR, { duration: 0.5, x: -eyeRX, y: -eyeRY, ease: 'expo.out' });
    gsap.to(nose, {
      duration: 0.5,
      x: -noseX,
      y: -noseY,
  ...
  const onEmailFocus = () => {
    state.current.activeElement = 'email';
    // stopBlinking();
    calculateFaceMove();
  };
  const onEmailBlur = () => {
  ...
  const spreadFingers = () => {
    if (!els.current.twoFingers) return;
    gsap.to(els.current.twoFingers, {
      duration: 0.35,
  ...
  const coverEyes = () => {
    const { armL, armR } = els.current;
    if (!armL || !armR) return;
    gsap.killTweensOf([armL, armR]);
    gsap.set([armL, armR], { css: { visibility: 'visible' } });
    gsap.to(armL, {
      duration: 0.45,
      x: -93,
      y: 10,
  ...
  const uncoverEyes = () => {
    ...
    gsap.to(armR, {
      ...
      delay: 0.1,
      onComplete: () => {
        gsap.set([armL, armR], { css: { visibility: 'hidden' } });
      },
    });
  ...
  // Initialize SVG element lookups once mounted
  useEffect(() => {
    const root = containerRef.current;
    if (!root) return;
    els.current.mySVG = root.querySelector('svg.mySVG');
    els.current.twoFingers = root.querySelector('.twoFingers');
    els.current.armL = root.querySelector('.armL');
    els.current.armR = root.querySelector('.armR');
    els.current.eyeL = root.querySelector('.eyeL');
    els.current.eyeR = root.querySelector('.eyeR');
    els.current.nose = root.querySelector('.nose');
    els.current.mouth = root.querySelector('.mouth');
    els.current.mouthBG = root.querySelector('.mouthBG');
    els.current.mouthSmallBG = root.querySelector('.mouthSmallBG');
    els.current.mouthMediumBG = root.querySelector('.mouthMediumBG');
    els.current.mouthLargeBG = root.querySelector('.mouthLargeBG');
    els.current.mouthMaskPath = root.querySelector('#mouthMaskPath');
    els.current.tooth = root.querySelector('.tooth');
    els.current.tongue = root.querySelector('.tongue');
    els.current.chin = root.querySelector('.chin');
    els.current.face = root.querySelector('.face');
    els.current.eyebrow = root.querySelector('.eyebrow');
    els.current.outerEarL = root.querySelector('.earL .outerEar');
    els.current.outerEarR = root.querySelector('.earR .outerEar');
    els.current.earHairL = root.querySelector('.earL .earHair');
    els.current.earHairR = root.querySelector('.earR .earHair');
    els.current.hair = root.querySelector('.hair');
    els.current.bodyBG = root.querySelector('.bodyBGnormal');
    els.current.bodyBGchanged = root.querySelector('.bodyBGchanged');

    // Move arms to initial hidden positions
    if (els.current.armL && els.current.armR) {
      gsap.set(els.current.armL, {
        x: -93,
        y: 220,
        rotation: 105,
        transformOrigin: 'top left',
      });
      gsap.set(els.current.armR, {
        x: -93,
        y: 220,
        rotation: -105,
        transformOrigin: 'top right',
      });
      gsap.set([els.current.armL, els.current.armR], {
        css: { visibility: 'hidden' },
      });
    }

    // Set initial mouth transform origin (positioning fix)
    if (els.current.mouth) {
      gsap.set(els.current.mouth, { transformOrigin: 'center center' });
    }
  ...
  // Attach input listeners to control the animation
  useEffect(() => {
    const email = emailInputRef?.current;
    const password = passwordInputRef?.current;
    if (!email) return;

    const focusEmail = () => onEmailFocus();
    const blurEmail = () => onEmailBlur();
    const inputEmail = () => onEmailInput();
    const keyupEmail = () => onEmailInput();
    const clickEmail = () => onEmailInput();

    email.addEventListener('focus', focusEmail);
    email.addEventListener('blur', blurEmail);
    email.addEventListener('input', inputEmail);
    email.addEventListener('keyup', keyupEmail);
    email.addEventListener('click', clickEmail);

    let blurTimeout: number | null = null;
    const focusPassword = () => {
      state.current.activeElement = 'password';
      if (!state.current.eyesCovered) coverEyes();
    };
    const blurPassword = () => {
      if (blurTimeout) window.clearTimeout(blurTimeout);
      blurTimeout = window.setTimeout(() => {
        if (state.current.activeElement === 'password') return;
        uncoverEyes();
      }, 120);
    };
    ...
    return () => {
      ...
      email.removeEventListener('blur', blurEmail);
      email.removeEventListener('input', inputEmail);
      email.removeEventListener('keyup', keyupEmail);
      email.removeEventListener('click', clickEmail);
      password?.removeEventListener('focus', focusPassword);
      password?.removeEventListener('blur', blurPassword);
    };
